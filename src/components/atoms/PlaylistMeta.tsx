...
const PlaylistMeta: React.FC<PlaylistMetaProps> = ({ targetPlaylist }) => {
	...
	const formattedCreationDate = dayjs(targetPlaylist?.creationDate).format(
		"D MMMM YYYY H:MM"
	);
	const getTotalSongsLength = useCallback(() => {
		if (targetPlaylist) {
			...
			return dayjs
				.duration(totalLength, "seconds")
				.format("HH:mm:ss [in total]");
		}
	}, [targetPlaylist]);
		...
		<div className="flex flex-col tracking-wide">
			...
			<div className="flex flex-col text-lg mt-3 text-menus-foreground-muted">
				...
				<p className="flex items-center gap-2">
					...
					{targetPlaylist?.songs.length}{" "}
					{targetPlaylist?.songs.length! > 1 ? "songs" : "song"},
					{` ${getTotalSongsLength()}`}
				</p>
				<p className="flex items-center gap-2">
